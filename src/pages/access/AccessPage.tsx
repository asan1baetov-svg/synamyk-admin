...
export function AccessPage() {
  ...
  const nothingSelected = (mode === 'test' && !testId) || (mode === 'user' && !userId)

  return (
    <div className="space-y-6">
      <PageHeader
        title="Доступы к тестам"
        description="Ручная выдача и отзыв доступа к тестам (покупается и выдаётся только тест целиком)"
      />
          ...
          <CardBody className="p-0">
            {loading ? (
              ...
              <p className="p-4 text-sm text-muted-foreground">
                {nothingSelected ? 'Выберите тест или пользователя.' : 'Нет выданных доступов.'}
              </p>
            ) : (
